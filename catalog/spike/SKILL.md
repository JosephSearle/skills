...
summary: Writes spikes as bounded, throwaway investigations, never implementations -- detects the right type from a SAFe/Agilemania taxonomy, enforces a falsifiable hypothesis and a concrete timebox with time/learning-bound acceptance criteria (never a working feature), and produces a findings-first, dense document with a validate-before-writing checklist, plus an update mode for enriching a spike after it runs and optional SAFe Enabler Story formatting.
---

# Spike Skill

A skill for creating and updating technical spike documents to industry standards. Grounded in
Extreme Programming origins (Kent Beck and Ron Jeffries), Mike Cohn's _User Stories Applied_ and
Mountain Goat Software writing, and the official Scaled Agile Framework (SAFe). See each
reference file's own citation list for what has been independently verified against a live
primary source vs. retained as this skill's own synthesis — a 2026-09 pass corrected one
fabricated citation (a "Sophie Déziel" article that could not be located) and one unverifiable
attribution (a "three-phase model" credited to Jeffries with no located source passage).
...
- [ ] If the spike is tied to an Epic, the Conclusion's Decision line states the Epic viability call, not just the narrow technical answer
- [ ] Approach section describes an investigation, not staged deliverable work — if it reads like sequential development wearing an "enabler" label, flag it per `references/safe-context.md` §0
...
- `references/document-format.md` — Section-level format rules, the density principle, findings-first conclusion structure, stub formats, prose style, and anti-patterns to flag and fix (note: previously mis-attributed to a non-existent "Sophie Déziel" source; now marked as skill-authored synthesis — see the file's own correction note)
