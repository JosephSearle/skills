...
| [webcraft-carbon-chat-ui](skills/webcraft-carbon-chat-ui/SKILL.md) | Build AI chat interfaces using Carbon + @carbon/ibm-products AILabel/Slug components and Vercel AI SDK: streaming message rendering, tool invocation display with InlineNotification, InlineLoading states, error handling, and the SSE proxy route handler pattern for LangGraph backends |
| [webcraft-carbon-dashboard](skills/webcraft-carbon-dashboard/SKILL.md) | Build data dashboards using @carbon/charts-react: chart type selection for AI operational metrics (latency→GaugeChart, throughput→LineChart, token usage→BarChart, trace flows→AlluvialChart), Carbon Grid/Column/Tile layout, TanStack Query live data with refetchInterval, and D3 for custom visualisations |
| [webcraft-carbon-setup](skills/webcraft-carbon-setup/SKILL.md) | Install and configure IBM Carbon Design System (@carbon/react v11) in a Next.js App Router project: SCSS barrel import, next.config.js sassOptions with includePaths, Theme provider with 'use client' boundary, dark/light mode switching, and design token usage instead of hardcoded values |
| [webcraft-forms-validation](skills/webcraft-forms-validation/SKILL.md) | Wire Carbon form components to React Hook Form v7 + Zod: Controller wrapper for TextInput, TextArea, Select, Dropdown, ComboBox, Checkbox, Toggle, DatePicker, and NumberInput; invalid/invalidText error display; multi-step forms with ProgressIndicator; InlineLoading on submit |
| [webcraft-nextjs-architecture](skills/webcraft-nextjs-architecture/SKILL.md) | Scaffold a Next.js 15 App Router project for IBM Carbon: server-shell/client-islands pattern, server/client boundary decision tree, Suspense streaming for LangGraph agent responses, route groups for chat/dashboard/general pages, and TypeScript strict mode with Bundler moduleResolution |
| [webcraft-openshift-deploy](skills/webcraft-openshift-deploy/SKILL.md) | Deploy a Next.js app on OpenShift: output:standalone config, multi-stage UBI9 Node 20 Dockerfile, arbitrary UID compliance (chmod -R g=u, USER 1001, HOSTNAME env var), /api/health liveness/readiness endpoint, ConfigMap/Secret env injection, and Tekton pipeline task outline |
| [webcraft-otel-frontend](skills/webcraft-otel-frontend/SKILL.md) | Add OpenTelemetry tracing to a Next.js frontend: OTel Web SDK + OTLP/HTTP exporter to MLflow, traceparent propagation to link browser spans with LangGraph agent spans, custom spans for agent interaction lifecycle, and Web Vitals reporting via reportWebVitals |
| [webcraft-state-management](skills/webcraft-state-management/SKILL.md) | Set up client and server state for Carbon Next.js apps: Zustand store with appendChunk streaming pattern and persist/devtools middleware, TanStack Query v5 provider with HydrationBoundary for SSR prefetching, refetchInterval polling, and optimistic updates |
| [webcraft-testing-setup](skills/webcraft-testing-setup/SKILL.md) | Configure Vitest + React Testing Library + Playwright for Carbon Next.js apps: jsdom ResizeObserver and matchMedia polyfills, RTL getByRole/getByLabelText query patterns for Carbon components, vi.mock('ai/react') useChat mock, and Playwright config with env-based baseURL for OpenShift staging |
| [webcraft-tooling-setup](skills/webcraft-tooling-setup/SKILL.md) | Configure developer tooling for a Carbon Next.js project: Biome (lint + format with a11y rules, replacing ESLint + Prettier), pre-commit with Biome + tsc + detect-secrets hooks, detect-secrets baseline workflow, and Storybook 8 with Carbon theme decorator and Vitest story-as-test integration |
