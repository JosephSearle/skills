...
| [itz-mcp-builder](skills/itz-mcp-builder/SKILL.md) | Develop and audit tools, resources, and prompts on NestJS MCP servers templated from the ITZ hardened template: enforces TechzoneAuthGuard JWT auth, two-layer CASL authorisation, per-user Redis rate limiting, Zod input validation, ToolBusinessError error channels, CloudEvents logging, and co-located Jest tests |
...
| [langchain-agents](skills/langchain-agents/SKILL.md) | Build, configure, and migrate to v1 agents using `create_agent`, `AgentMiddleware`, `system_prompt`, and `response_format`; covers all 15+ built-in middlewares (PIIMiddleware, SummarizationMiddleware, HumanInTheLoopMiddleware, ModelCallLimitMiddleware, ToolRetryMiddleware, AnthropicPromptCachingMiddleware, and more), `AgentState` TypedDict extension rules, and full migration from deprecated `AgentExecutor` and `create_react_agent` |
| [langchain-core](skills/langchain-core/SKILL.md) | LangChain Core & LCEL — Runnables, `init_chat_model` (all 26 provider prefixes), pipe syntax, `astream_events`, `RunnableConfig` with COPIABLE_KEYS semantics, `with_structured_output` strategies, `ChatPromptTemplate`, `BaseCallbackHandler`, `CacheBackedEmbeddings`, and migration from deprecated chains (`LLMChain`, `RetrievalQA`, `AgentExecutor`) to modern LCEL |
...
| [langchain-rag](skills/langchain-rag/SKILL.md) | Build and optimise production RAG pipelines with LangChain: idempotent ingestion via SQLRecordManager and index()/aindex(), per-store reference for PGVectorStore, Pinecone, Qdrant, Milvus, MongoDB Atlas, Elasticsearch, Redis, and Weaviate, chunking strategy selection (RCTS/TokenTextSplitter/SemanticChunker/MarkdownHeaderTextSplitter), retriever patterns (MMR, EnsembleRetriever, ParentDocumentRetriever, SelfQueryRetriever), two-stage reranking (CohereRerank, CrossEncoderReranker), and advanced RAG architectures as LangGraph StateGraphs (Self-RAG, CRAG, HyDE, Adaptive RAG) |
...
| [langgraph-core](skills/langgraph-core/SKILL.md) | LangGraph runtime — `StateGraph`, checkpointers (`PostgresSaver`, `InMemorySaver`), HITL (`interrupt()`, `Command(resume=`), all 7 `stream_mode` values with payloads, `BaseStore`, `PostgresStore`, `@entrypoint`/`@task` functional API, time-travel with `get_state_history`/`update_state`, subgraph composition, and `Send` for map-reduce fan-out |
| [langgraph-memory](skills/langgraph-memory/SKILL.md) | Memory management for LangGraph agents — `trim_messages`, `SummarizationNode`, `LangMem` (0.0.x), `PostgresStore`, cross-thread memory via `InjectedStore`, short-term context overflow handling (HARD ERROR, issue #3717), long-term store patterns, `ReflectionExecutor` background consolidation, and migration from `ConversationBufferMemory`/`ConversationSummaryMemory` |
| [langgraph-multiagent](skills/langgraph-multiagent/SKILL.md) | Multi-agent system patterns — supervisor, swarm, `create_deep_agent` (deepagents 0.6.7), `RemoteGraph`, `langgraph-bigtool`, `create_handoff_tool`, `Command(goto=`, `Command.PARENT`, `SubAgentMiddleware`, `SkillsMiddleware`, `AgentsMdMiddleware`, and the 5 official LangChain patterns with token call-count benchmarks |
| [langsmith-core](skills/langsmith-core/SKILL.md) | LangSmith observability, evaluation, and prompt management — `@traceable`, both `LANGCHAIN_*` and `LANGSMITH_*` env var aliases, `evaluate()`/`aevaluate()` (max_concurrency=0 gotcha), `create_llm_as_judge`, `create_trajectory_match_evaluator`, dataset management, `client.pull_prompt`/`client.push_prompt`, online evaluators, Multi-turn Evals, annotation queues, and OTel ingestion |
| [langsmith-deployment](skills/langsmith-deployment/SKILL.md) | LangGraph Agent Server deployment — all 4 topologies (Cloud/Self-Hosted/BYOC/Lite), `langgraph.json` schema, `langgraph deploy` CLI, `LangGraphClient`/`AsyncLangGraphClient`, `RemoteGraph`, `multitask_strategy` (all 4 values), streaming SSE, Kubernetes Helm chart, `N_JOBS_PER_WORKER`, `agent-chat-ui`, and Postgres/Redis data ownership |
