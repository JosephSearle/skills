...
| [langgraph-multiagent](skills/langgraph-multiagent/SKILL.md) | Multi-agent system patterns — supervisor, swarm, `create_deep_agent` (deepagents 0.6.7), `RemoteGraph` (see `langgraph-deployment` for self-hosted server), `langgraph-bigtool`, `create_handoff_tool`, `Command(goto=`, `Command.PARENT`, `SubAgentMiddleware`, `SkillsMiddleware`, `AgentsMdMiddleware`, the 5 official LangChain patterns with token call-count benchmarks, and MLflow multi-agent span tracing |
